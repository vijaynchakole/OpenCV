# -*- coding: utf-8 -*-
"""
Created on Wed May  6 18:07:15 2020
...
width = test_digits.shape[1] # x-axis
height = test_digits.shape[0] # y-axis
dim = (width,height) # (x,y)

test_img = cv2.imread("four.png", cv2.IMREAD_GRAYSCALE)

test_img.shape

test_img_resized = cv2.resize(test_img, dim, interpolation = cv2.INTER_AREA)

test_img_resized = test_img_resized.flatten()
test_img_resized = np.array(test_img_resized, dtype = np.float32)
...
show_img_with_matplotlib(cv2.cvtColor(test_img, cv2.COLOR_GRAY2RGB), "")

show_img_with_matplotlib(cv2.cvtColor(test_img_resized, cv2.COLOR_GRAY2RGB), "")
test_img_resized
...
ret, result, neighbours, dist = knn.findNearest(test_img_resized, k = 3)
